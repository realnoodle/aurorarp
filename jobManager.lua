...
AddEventHandler('es_rp:cuff', function(user)
	if POLICE[jobs[source].name] then
		...
		TriggerClientEvent('es_rp:notify', source, "~r~You're not a law enforcement officer.")
	end
end)
...
AddEventHandler('es_rp:spawnPlayer', function()
	...
	local job = jobs[Source] or JOB_UNEMPLOYED
		...
		if GetPlayerName(tonumber(args[2])) then
			if deadPeople[tonumber(args[2])]then
				TriggerEvent('es:getPlayerFromId', tonumber(args[2]), function(target)
					...
					local job = jobs[tonumber(args[2])] or JOB_UNEMPLOYED
		...
		if GetPlayerName(tonumber(args[2])) then
			if deadPeople[tonumber(args[2])]then
				TriggerEvent('es:getPlayerFromId', tonumber(usar), function(target)
					...
					local job = jobs[tonumber(usar)] or JOB_UNEMPLOYED
					local spawn = target.getCoords()
	...
	if JOBS[job] then
		TriggerEvent('es:getPlayerFromId', source, function(user)
			if JOBS[job].permission_level <= user.getPermissions() then
				jobs[source] = JOBS[job] or JOB_UNEMPLOYED
	...
	TriggerEvent('es:getPlayers', function(players)
		for i in pairs(players)do
			if jobs[players[i].get('source')] == nil then
				jobs[players[i].get('source')] = JOB_UNEMPLOYED
				TriggerClientEvent('es_rp:setJob', source, jobs[players[i].get('source')].name)
			end

			players[i].addBank(jobs[players[i].get('source')].salary)
          	
          	if jobs[players[i].get('source')].name == "Unemployed" then TriggerClientEvent('es_rp:notify', players[i].get('source'), "You've received your welfare ~g~$" .. jobs[players[i].get('source')].salary) end
			if jobs[players[i].get('source')].name ~= "Unemployed" then TriggerClientEvent('es_rp:notify', players[i].get('source'), "You've received your paycheck ~g~$" .. jobs[players[i].get('source')].salary) end
		end
	end)
